...
const RegisterPage = () => {
  ...
  const schema = yup.object().shape({
    email: yup.string().email(t(`Email format is invalid`)).required(t('Email is required')),
    password: yup.string().required(t('Password is required')).min(6, t(`Password must be at least {{n}} characters`, {'n': 6})),
    agree: yup
      .boolean()
  ...
  useEffect(() => {
    ...
    if (isRegisterError && typeof isRegisterError.response?.data === 'object') {
      if(isRegisterError?.response?.data) {
        Object.entries(isRegisterError.response.data).forEach(([key, value]) => {
          if(value) {
    ...
    if (isRegisterLoaded) {
      Notify(
        'success',
        t(`Account created! Find the email in your mailbox and click on the link to complete the registration`)
      );
      dispatch(resetRegisterState());
      router.push('/login');
    }
